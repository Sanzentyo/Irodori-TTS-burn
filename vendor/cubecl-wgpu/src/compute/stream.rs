...
use crate::{
    ...
    schedule::{Addresses, BindingsResource, ScheduleTask},
};
use core::iter;
#[cfg(renderdoc)]
...
pub(crate) struct CapturedDispatch {
    ...
    count: CapturedCubeCount,
    bind_group: Option<wgpu::BindGroup>,
    custom_resources: Vec<WgpuResource>,
    addresses: Option<Addresses>,
}

#[derive(Debug)]
enum CapturedCubeCount {
    Static(u32, u32, u32),
    Dynamic(WgpuResource),
}

impl CapturedCubeCount {
    fn is_empty(&self) -> bool {
        matches!(self, Self::Static(x, y, z) if *x == 0 || *y == 0 || *z == 0)
    }
}
...
pub struct WgpuStream {
    ...
    graph_mem_manage: Option<WgpuMemManager>,
...
impl WgpuStream {
    ...
    pub(crate) fn begin_software_graph_memory(&mut self) -> Result<(), ServerError> {
        if self.graph_mem_manage.is_some() {
            return Err(ServerError::Generic {
                reason: "WGPU software graph memory is already active".to_owned(),
                backtrace: BackTrace::capture(),
            });
        }
        let graph_memory =
    ...
    pub(crate) fn take_software_graph_memory(&mut self) -> Option<WgpuMemManager> {
        self.graph_mem_manage.take()
    }

    pub(crate) fn empty_software_graph(
        &mut self,
        size: u64,
    ) -> Result<ManagedMemoryHandle, IoError> {
        self.graph_mem_manage
            .as_mut()
            .expect("software graph memory must be active")
            .reserve(size)
    }

    pub(crate) fn bind_software_graph(
        &mut self,
    ...
    pub(crate) fn get_active_resource(
        &mut self,
        binding: cubecl_core::server::Binding,
    ) -> Result<WgpuResource, IoError> {
        if let Some(graph) = self.graph_mem_manage.as_mut()
            && let Ok(resource) = graph.get_resource(binding.clone())
        {
            return Ok(resource);
        }
        self.mem_manage.get_resource(binding)
    }
    ...
    pub(crate) fn create_uniform(&mut self, data: &[u8]) -> WgpuResource {
        let resource = match self.graph_mem_manage.as_mut() {
            Some(graph) => graph.reserve_uniform(data.len() as u64),
            None => self.mem_manage.reserve_uniform(data.len() as u64),
        };
    ...
    pub fn flush(&mut self, mode: StreamErrorMode) -> Result<(), ServerError> {
        ...
        if let Some(graph) = self.graph_mem_manage.as_mut() {
            // Keep graph-owned device pages alive, but make completed
            // metadata uniforms reusable during the priming pass.
            graph.release_uniforms();
        }
    ...
    pub(crate) fn prepare_captured_dispatch(
        &mut self,
        pipeline: Arc<ComputePipeline>,
        count: CubeCount,
        resources: BindingsResource,
    ) -> CapturedDispatch {
        let (resources, custom_resources, addresses) = resources.into_resources(self);
        let bind_group = if resources.is_empty() {
            None
        } else {
            ...
            Some(self.device.create_bind_group(&wgpu::BindGroupDescriptor {
                label: Some("CubeCL WGPU software graph"),
                layout: &layout,
                entries: &entries,
            }))
        };
        let count = match count {
            CubeCount::Static(x, y, z) => CapturedCubeCount::Static(x, y, z),
            CubeCount::Dynamic(binding) => CapturedCubeCount::Dynamic(
                self.get_active_resource(binding)
                    .expect("captured indirect dispatch buffer must remain available"),
            ),
        };
        CapturedDispatch {
            pipeline,
    ...
    pub(crate) fn replay_captured_dispatch(&mut self, dispatch: &CapturedDispatch) {
        if dispatch.count.is_empty() {
            return;
        }
        let pass = self.compute_pass.get_or_insert_with(|| {
            ...
            self.encoder
                .begin_compute_pass(&wgpu::ComputePassDescriptor {
                    label: Some("CubeCL WGPU software graph replay"),
        ...
        if let Some(bind_group) = &dispatch.bind_group {
            pass.set_bind_group(0, bind_group, &[]);
        }
        if let Some(addresses) = &dispatch.addresses {
            pass.set_immediates(0, bytemuck::cast_slice(addresses));
        }
        if !dispatch.custom_resources.is_empty() {
            let transitions = dispatch
                .custom_resources
                .iter()
            ...
            pass.transition_resources(transitions, iter::empty());
        }
        match &dispatch.count {
            CapturedCubeCount::Static(x, y, z) => pass.dispatch_workgroups(*x, *y, *z),
            CapturedCubeCount::Dynamic(resource) => {
                pass.dispatch_workgroups_indirect(&resource.buffer, resource.offset);
            }
        }
        // Preserve the runtime's measured submission cadence. Software graph
        // replay removes host-side graph construction; it does not assume one
        // giant command buffer is faster on every WGPU driver.
