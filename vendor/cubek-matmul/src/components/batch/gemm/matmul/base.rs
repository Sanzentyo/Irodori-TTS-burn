...
use crate::components::batch::gemm::matmul::dot::{execute_dot, execute_pairwise_dot};
...
use crate::components::batch::{
    ...
    gemm::{
        GemmBlueprint, GemmConfig, GemmFamily, PairwiseGemmFamily, PlanesSplit, Variant,
    },
};
...
#[cube(launch_unchecked, explicit_define, address_type = "dynamic")]
#[allow(clippy::type_complexity)]
/// Pairwise-compressed GEMM entry point.
pub fn pairwise_matmul_entry<
    Args: MatmulArgs<Config = ()>,
    Lhs: Numeric,
    LhsSize: Size,
    Rhs: Numeric,
...
>(
    ...
    let device_props = comptime::device_properties();
    let config = comptime!(PairwiseGemmFamily::<E>::expand_config(
        &device_props,
        &blueprint,
        &blueprint.dtypes,
        &vector_sizes
...
pub struct PairwiseGemm<MP: MatmulTypes, E: PairwiseAccumulatorGlobalEpilogue<()>> {
    _phantom: PhantomData<(MP, E)>,
}

#[cube]
impl<MP, E> BatchMatmul<(), MP> for PairwiseGemm<MP, E>
where
    MP: MatmulTypes,
    E: PairwiseAccumulatorGlobalEpilogue<()>,
{
    ...
    fn execute<Args: MatmulArgs>(
        ...
        let (_, m, k) = lhs.shape();
        let (_, _, logical_n) = rhs.shape();
        let physical_n = logical_n / 2;
        ...
        let rhs_view = rhs.view(MatLayout::new(rhs_batch, (k, logical_n)));
        let out_view = out.view_mut(MatLayout::new(out_batch, (m, physical_n)));
        let logical_n_pos = cube_n * config.num_planes + UNIT_POS_Y;
        execute_pairwise_dot::<
            LhsG<MP>,
            RhsG<MP>,
            Global<Acc<MP>>,
            GlobalSize<Acc<MP>>,
