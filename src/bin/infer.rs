...
struct Args {
    ...
    ///
    /// Overridden by `--noise-file` if both are supplied.
    #[arg(long)]
    seed: Option<u64>,
...
///
/// Expects a tensor named `"initial_noise"` with shape `[batch, seq_len, dim]`.
/// The file is generated by `scripts/export_initial_noise.py`.
fn load_initial_noise<B: Backend>(
    ...
    anyhow::ensure!(
        shape[1] == seq_len,
        "initial_noise seq_len={} but requested seq_len={}; re-export with --seq-len {}",
        shape[1],
        seq_len,
        seq_len,
    );
