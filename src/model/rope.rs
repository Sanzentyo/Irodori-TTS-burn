...
impl RopeFreqs {
    pub fn new(cos: Tensor<2>, sin: Tensor<2>) -> Self {
        Self { cos, sin }
    }
}
...
pub fn precompute_rope_freqs_typed(
    head_dim: usize,
    seq_len: usize,
    theta: f64,
    device: &Device,
...
pub fn apply_rotary_half(x: Tensor<4>, cos: Tensor<2>, sin: Tensor<2>) -> Tensor<4> {
    let [batch, seq, heads, head_dim] = x.dims();
    let half_heads = heads / 2;

    // Split along heads dimension (dim 2)
    let x_rot: Tensor<4> = x
        .clone()
        .slice([0..batch, 0..seq, 0..half_heads, 0..head_dim]);
    let x_pass: Tensor<4> = x.slice([0..batch, 0..seq, half_heads..heads, 0..head_dim]);
...
/// Returns: `[batch, dim]`
pub fn get_timestep_embedding(timestep: Tensor<1>, dim: usize, device: &Device) -> Tensor<2> {
...
mod tests {
    ...
    fn precompute_rope_freqs_shape() {
        ...
        let (cos, sin) = precompute_rope_freqs(8, 16, 10000.0, &device);
    ...
    fn rope_position_zero_is_identity() {
        let device = Default::default();
        let (cos, sin) = precompute_rope_freqs(4, 1, 10000.0, &device);
    ...
    fn apply_rotary_emb_preserves_shape() {
        let device = Default::default();
        let (cos, sin) = precompute_rope_freqs(8, 4, 10000.0, &device);
        let x = Tensor::<4>::ones([2, 4, 3, 8], &device);
    ...
    fn apply_rotary_emb_position_zero_identity() {
        ...
        let x = Tensor::<4>::from_floats([[[[1.0, 2.0, 3.0, 4.0]]]], &device);
    ...
    fn apply_rotary_half_preserves_shape() {
        let device = Default::default();
        let (cos, sin) = precompute_rope_freqs(8, 4, 10000.0, &device);
        let x = Tensor::<4>::ones([2, 4, 6, 8], &device); // 6 heads
    ...
    fn apply_rotary_half_second_half_unchanged() {
        let device = Default::default();
        let (cos, sin) = precompute_rope_freqs(4, 2, 10000.0, &device);
        let x = Tensor::<4>::ones([1, 2, 4, 4], &device) * 5.0; // 4 heads
        let y = apply_rotary_half(x, cos, sin);
        // The second half of heads (heads 2,3) should be unchanged
        let pass: Tensor<4> = y.slice([0..1, 0..2, 2..4, 0..4]);
    ...
    fn timestep_embedding_shape() {
        let device = Default::default();
        let t = Tensor::<1>::from_floats([0.0, 0.5, 1.0], &device);
    ...
    fn timestep_embedding_different_for_different_times() {
        let device = Default::default();
        let t0 = Tensor::<1>::from_floats([0.0], &device);
        let t1 = Tensor::<1>::from_floats([1.0], &device);
        let e0 = get_timestep_embedding(t0, 8, &device);
        let e1 = get_timestep_embedding(t1, 8, &device);
        let diff = (e0 - e1).abs().sum().into_scalar::<f32>();
        assert!(
            diff > 1e-3,
