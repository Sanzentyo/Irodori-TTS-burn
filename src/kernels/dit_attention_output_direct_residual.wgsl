// Direct JointAttention tail for the released v4-Small geometry.
//
// SDPA produces contiguous [B,H,S,Dh], while the output projection consumes
// token-major [B,S,D].  Fold that layout transform and the learned sigmoid
...
const GATE_ROW_STRIDE: u32 = {{ gate_row_stride }}u;
    ...
    for (var k_base = 0u; k_base < K; k_base = k_base + TILE_K) {
        for (var load = local_index; load < TILE_ROWS * TILE_K; load = load + 256u) {
            ...
            if (row < ROWS) {
                ...
                let dimension = k_base + tile_k;
                let head = dimension / DH;
                let component = dimension - head * DH;
                let attention_index = ((batch * H + head) * SEQUENCE + sequence) * DH + component;
                let gate_index = row * GATE_ROW_STRIDE + GATE_OFFSET + dimension;
                value = attention[attention_index] * attention_gate[gate_index];
            }
