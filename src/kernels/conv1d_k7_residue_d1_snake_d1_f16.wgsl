...
// Production residue-class dilation-one k=7 Conv1d + scalar Snake with vec4 output stores.
//
...
@group(0) @binding(0) var<storage, read_write> packed_input: array<f16>;
@group(0) @binding(1) var<storage, read_write> weight_buf: array<vec4<f16>>;
...
@group(0) @binding(4) var<storage, read_write> alpha_buf: array<f16>;
...
fn snake_epilogue(x: f32, output_channel: u32) -> f32 {
    let a = f32(alpha_buf[output_channel]);
    let sine = sin(a * x);
...
fn store_output_vec4(
    ...
    if q < q_length {
        ...
        let output_index = output_base + output_time;
        output_buf[output_index / 4u] = vec4<f16>(
            f16(snake_epilogue(value.x, output_channel)),
            f16(snake_epilogue(value.y, output_channel)),
            f16(snake_epilogue(value.z, output_channel)),
            f16(snake_epilogue(value.w, output_channel)),
        );
    }
}
...
fn main(
    ...
    let bias_2 = f32(bias_buf[output_channel_2]);
    let bias_3 = f32(bias_buf[output_channel_3]);
    ...
    loop {
        ...
        loop {
            ...
            if source_q >= 0 && source_q < i32(q_length) {
                ...
                let packed_index = packed_residue_base
                    ...
                    + u32(source_q);
                value = f32(packed_input[packed_index]);
            }
            input_tile[tile_index] = value;
            tile_index += WORKGROUP_SIZE;
        }

        // Stage the invocation-owned output vectors prepared once at load time.
        ...
        loop {
            ...
            let output_vector =
                group_id.y * (OUTPUT_CHANNEL_TILE / 4u) + tile_output_vector;
            let weight_index =
                (input_channel * KERNEL_SIZE + kernel_index) * OUTPUT_VECTORS + output_vector;
            weight_tile[tile_index] = vec4<f32>(weight_buf[weight_index]);
            tile_index += WORKGROUP_SIZE;
        }
        ...
        loop {
            ...
            // tap 0
            var weight_vector = weight_tile[weight_base];
            var weight_0 = weight_vector.x;
            var weight_1 = weight_vector.y;
            var weight_2 = weight_vector.z;
            var weight_3 = weight_vector.w;
            ...
            // tap 1
            weight_vector = weight_tile[weight_base + 1u];
            weight_0 = weight_vector.x;
            weight_1 = weight_vector.y;
            weight_2 = weight_vector.z;
            weight_3 = weight_vector.w;
            ...
            // tap 2
            weight_vector = weight_tile[weight_base + 2u];
            weight_0 = weight_vector.x;
            weight_1 = weight_vector.y;
            weight_2 = weight_vector.z;
            weight_3 = weight_vector.w;
            ...
            // tap 3
            weight_vector = weight_tile[weight_base + 3u];
            weight_0 = weight_vector.x;
            weight_1 = weight_vector.y;
            weight_2 = weight_vector.z;
            weight_3 = weight_vector.w;
            ...
            // tap 4
            weight_vector = weight_tile[weight_base + 4u];
            weight_0 = weight_vector.x;
            weight_1 = weight_vector.y;
            weight_2 = weight_vector.z;
            weight_3 = weight_vector.w;
            ...
            // tap 5
            weight_vector = weight_tile[weight_base + 5u];
            weight_0 = weight_vector.x;
            weight_1 = weight_vector.y;
            weight_2 = weight_vector.z;
            weight_3 = weight_vector.w;
            ...
            // tap 6
            weight_vector = weight_tile[weight_base + 6u];
            weight_0 = weight_vector.x;
            weight_1 = weight_vector.y;
            weight_2 = weight_vector.z;
            weight_3 = weight_vector.w;
