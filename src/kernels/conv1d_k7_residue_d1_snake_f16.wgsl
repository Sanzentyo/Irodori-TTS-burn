...
fn store_output_vec4(
    ...
    if q < q_length {
        ...
        output_buf[output_base + output_time] = f16(snake_epilogue(value.x, output_channel));
    }
    if q + 1u < q_length {
        let output_time = residue + (q + 1u) * DILATION;
        output_buf[output_base + output_time] = f16(snake_epilogue(value.y, output_channel));
    }
    if q + 2u < q_length {
        let output_time = residue + (q + 2u) * DILATION;
        output_buf[output_base + output_time] = f16(snake_epilogue(value.z, output_channel));
    }
    if q + 3u < q_length {
        let output_time = residue + (q + 3u) * DILATION;
        output_buf[output_base + output_time] = f16(snake_epilogue(value.w, output_channel));
    }
}
