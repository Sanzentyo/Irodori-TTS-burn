...
// F16-storage K16 MLP expansion with register-prefetched global tiles.
//
// One input vec4 and two gate/value weight vec4s are prefetched per
// invocation before the shared-page overwrite barrier. This keeps the 12 KiB
// workgroup footprint while overlapping next-tile global loads with useful
// work from the rest of the group.
...
fn load_input_value(load: u32, k_base: u32, row_base: u32) -> vec4<f32> {
    ...
    if (row < ROWS) {
        return vec4<f32>(input[row * K_VECS + k_base / 4u + tile_k_vec]);
    }
    return vec4<f32>(0.0);
}

fn load_weight_value(load: u32, k_base: u32, hidden_vec_base: u32) -> vec4<f32> {
    ...
    if (hidden_vec < HIDDEN_VECS) {
        ...
        return vec4<f32>(weight[(k_base + tile_k) * EXPANDED_VECS + expanded_vec]);
    }
    return vec4<f32>(0.0);
}
...
fn main(
    ...
    var prefetched_weight_0 = load_weight_value(weight_load_0, 0u, hidden_vec_base);
    var prefetched_weight_1 = load_weight_value(weight_load_1, 0u, hidden_vec_base);
    ...
    for (var k_base = 0u; k_base < K; k_base = k_base + TILE_K) {
        ...
        if (next_k < K) {
            ...
            prefetched_weight_0 = load_weight_value(weight_load_0, next_k, hidden_vec_base);
            prefetched_weight_1 = load_weight_value(weight_load_1, next_k, hidden_vec_base);
            workgroupBarrier();
            input_tile[input_load] = prefetched_input;
            weight_tile[weight_load_0] = prefetched_weight_0;
            weight_tile[weight_load_1] = prefetched_weight_1;
