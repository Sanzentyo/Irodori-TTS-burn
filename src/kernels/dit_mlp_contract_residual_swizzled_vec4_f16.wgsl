enable f16;
// Exact FP32 MLP contract with bank-aware shared-weight staging.
//
// Global weights remain row-major vec4. Inside each K row, the four output
// components are transposed to [component][column_vec], so one component read
// by adjacent lanes addresses adjacent 32-bit banks instead of a four-word
// stride. FMA and epilogue order match the established vector route.
...
var<workgroup> weight_tile: array<f32, {{ weight_tile_scalars }}>;

fn staged_weight(tile_k: u32, column_vec: u32) -> vec4<f32> {
    let base = tile_k * TILE_COLUMNS + column_vec;
    return vec4<f32>(
        weight_tile[base],
        weight_tile[base + LOCAL_COLUMN_VECS],
        weight_tile[base + 2u * LOCAL_COLUMN_VECS],
        weight_tile[base + 3u * LOCAL_COLUMN_VECS],
    );
}
...
fn store_result(row: u32, column_vec: u32, branch: vec4<f32>) {
    if (row < ROWS && column_vec < N_VECS) {
        ...
        output[index] = vec4<f16>(vec4<f32>(residual[index]) + vec4<f32>(gate[batch * N_VECS + column_vec]) * branch);
    }
}
...
fn main(
    ...
    for (var k_base = 0u; k_base < K; k_base = k_base + TILE_K) {
        ...
        for (var load = local_index; load < TILE_K * LOCAL_COLUMN_VECS; load = load + 256u) {
            ...
            let base = tile_k * TILE_COLUMNS + tile_column_vec;
            weight_tile[base] = value.x;
            weight_tile[base + LOCAL_COLUMN_VECS] = value.y;
            weight_tile[base + 2u * LOCAL_COLUMN_VECS] = value.z;
            weight_tile[base + 3u * LOCAL_COLUMN_VECS] = value.w;
        }
        workgroupBarrier();
        ...
        for (var tile_k_vec = 0u; tile_k_vec < TILE_K_VECS; tile_k_vec = tile_k_vec + 1u) {
            ...
            let weight_00 = staged_weight(k0, local_id.x);
            let weight_10 = staged_weight(k0, local_id.x + 16u);
            ...
            let weight_01 = staged_weight(k0 + 1u, local_id.x);
            let weight_11 = staged_weight(k0 + 1u, local_id.x + 16u);
            ...
            let weight_02 = staged_weight(k0 + 2u, local_id.x);
            let weight_12 = staged_weight(k0 + 2u, local_id.x + 16u);
            ...
            let weight_03 = staged_weight(k0 + 3u, local_id.x);
            let weight_13 = staged_weight(k0 + 3u, local_id.x + 16u);
