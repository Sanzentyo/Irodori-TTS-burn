...
fn main(
    ...
    if output_channel_0 < CHANNELS {
        bias_0 = f32(bias_buf[output_channel_0]);
    }
    if output_channel_1 < CHANNELS {
        bias_1 = f32(bias_buf[output_channel_1]);
    }
    if output_channel_2 < CHANNELS {
        bias_2 = f32(bias_buf[output_channel_2]);
    }
    if output_channel_3 < CHANNELS {
        bias_3 = f32(bias_buf[output_channel_3]);
    }
    ...
    loop {
        ...
        loop {
            ...
            if output_channel < CHANNELS {
                ...
                value = f32(weight_buf[weight_index]);
            }
    ...
    if output_channel_0 < CHANNELS {
        ...
        if time_0 < LENGTH {
            output_buf[output_base + time_0] = f16(accumulator_00);
        }
        if time_1 < LENGTH {
            output_buf[output_base + time_1] = f16(accumulator_10);
        }
        if time_2 < LENGTH {
            output_buf[output_base + time_2] = f16(accumulator_20);
        }
        if time_3 < LENGTH {
            output_buf[output_base + time_3] = f16(accumulator_30);
        }
    }
    if output_channel_1 < CHANNELS {
        let output_base = (batch_index * CHANNELS + output_channel_1) * LENGTH;
        if time_0 < LENGTH {
            output_buf[output_base + time_0] = f16(accumulator_01);
        }
        if time_1 < LENGTH {
            output_buf[output_base + time_1] = f16(accumulator_11);
        }
        if time_2 < LENGTH {
            output_buf[output_base + time_2] = f16(accumulator_21);
        }
        if time_3 < LENGTH {
            output_buf[output_base + time_3] = f16(accumulator_31);
        }
    }
    if output_channel_2 < CHANNELS {
        let output_base = (batch_index * CHANNELS + output_channel_2) * LENGTH;
        if time_0 < LENGTH {
            output_buf[output_base + time_0] = f16(accumulator_02);
        }
        if time_1 < LENGTH {
            output_buf[output_base + time_1] = f16(accumulator_12);
        }
        if time_2 < LENGTH {
            output_buf[output_base + time_2] = f16(accumulator_22);
        }
        if time_3 < LENGTH {
            output_buf[output_base + time_3] = f16(accumulator_32);
        }
    }
    if output_channel_3 < CHANNELS {
        let output_base = (batch_index * CHANNELS + output_channel_3) * LENGTH;
        if time_0 < LENGTH {
            output_buf[output_base + time_0] = f16(accumulator_03);
        }
        if time_1 < LENGTH {
            output_buf[output_base + time_1] = f16(accumulator_13);
        }
        if time_2 < LENGTH {
            output_buf[output_base + time_2] = f16(accumulator_23);
        }
        if time_3 < LENGTH {
            output_buf[output_base + time_3] = f16(accumulator_33);
        }
    }
}
