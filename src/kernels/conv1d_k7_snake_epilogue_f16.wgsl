...
// T64 tiled k=7 Conv1d + ResidualUnit act1 Snake1d epilogue.
//
// This is the common body of the accepted T64/O16/WG128 and
// T64/O32/WG256 kernels. The tile-specific values are source-template
// constants so the convolution reduction order remains exactly
// input-channel-then-kernel for both variants.
//
// Physical layouts:
...
// Keep this operation sequence identical to snake.wgsl. The convolution
// accumulator is exactly the standalone Snake kernel's `x` input.
...
fn main(
    ...
    var accumulator_00 = f32(bias_buf[output_channel_0]);
    var accumulator_01 = f32(bias_buf[output_channel_1]);
    ...
    if time_0 < LENGTH {
        output_buf[output_base_0 + time_0] = f16(snake_epilogue(accumulator_00, output_channel_0));
        output_buf[output_base_1 + time_0] = f16(snake_epilogue(accumulator_01, output_channel_1));
    }
    if time_1 < LENGTH {
        output_buf[output_base_0 + time_1] = f16(snake_epilogue(accumulator_10, output_channel_0));
        output_buf[output_base_1 + time_1] = f16(snake_epilogue(accumulator_11, output_channel_1));
    }
    if time_2 < LENGTH {
        output_buf[output_base_0 + time_2] = f16(snake_epilogue(accumulator_20, output_channel_0));
        output_buf[output_base_1 + time_2] = f16(snake_epilogue(accumulator_21, output_channel_1));
    }
    if time_3 < LENGTH {
        output_buf[output_base_0 + time_3] = f16(snake_epilogue(accumulator_30, output_channel_0));
        output_buf[output_base_1 + time_3] = f16(snake_epilogue(accumulator_31, output_channel_1));
    }
}
