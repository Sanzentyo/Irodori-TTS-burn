// One physical latent row is projected once and written to every B2/B3 CFG
// row. A[ROWS,32] and the checkpoint-native B[32,1280] are row-major.
...
fn main(
    ...
    for (var k_base = 0u; k_base < K; k_base = k_base + TILE_K) {
        ...
        let weight_value = weight[(k_base + load_k) * N_VECS + column_vec_base + load_output_vec];
    ...
    for (var owned_row = 0u; owned_row < 4u; owned_row = owned_row + 1u) {
        ...
        if (row < ROWS) {
            for (var batch = 0u; batch < BATCH; batch = batch + 1u) {
                output[(batch * ROWS + row) * N_VECS + output_column_vec] = values[owned_row];
            }
        }
    }
}
