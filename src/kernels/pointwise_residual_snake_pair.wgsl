...
@group(0) @binding(0) var<storage, read_write> branch_nlc: array<f32>;
@group(0) @binding(1) var<storage, read_write> bias: array<f32>;
@group(0) @binding(2) var<storage, read_write> residual_ncl: array<f32>;
@group(0) @binding(3) var<storage, read_write> alpha: array<f32>;
@group(0) @binding(4) var<storage, read_write> raw_ncl: array<f32>;
@group(0) @binding(5) var<storage, read_write> activated_ncl: array<f32>;
...
fn main(@builtin(global_invocation_id) global_id: vec3<u32>) {
    ...
    let biased = branch_nlc[branch_index] + bias[channel];
    let raw = biased + residual_ncl[output_index];
    raw_ncl[output_index] = raw;

    let a = alpha[channel];
    let sine = sin(a * raw);
    activated_ncl[output_index] = raw + (sine * sine) / (a + 1e-9);
}
