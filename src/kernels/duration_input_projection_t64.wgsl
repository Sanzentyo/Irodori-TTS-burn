...
@group(0) @binding(2) var<storage, read_write> bias: array<vec4<f32>>;
...
fn main(
    ...
    for (var k_base = 0u; k_base < K; k_base = k_base + TILE_K) {
        ...
        let weight_value = weight[(column_base + load_output) * K_VECS + k_base / 4u + load_k_vec];
    ...
    let bias_value = bias[output_column_vec];
    ...
    if (output_row_0 < ROWS) {
        output[output_row_0 * N_VECS + output_column_vec] = acc_0 + bias_value;
    }
    if (output_row_1 < ROWS) {
        output[output_row_1 * N_VECS + output_column_vec] = acc_1 + bias_value;
    }
    if (output_row_2 < ROWS) {
        output[output_row_2 * N_VECS + output_column_vec] = acc_2 + bias_value;
    }
    if (output_row_3 < ROWS) {
        output[output_row_3 * N_VECS + output_column_vec] = acc_3 + bias_value;
    }
}
