// Exact FP32 MLP contract with a 48-row occupancy tile, vectorized K staging,
// and a gated-residual epilogue. It sits between the 32-row and 64-row routes:
// more independent workgroups than the latter without doubling repeated
// weight traffic like the former.
...
fn main(
    ...
    for (var k_base = 0u; k_base < K; k_base = k_base + TILE_K) {
        ...
        for (var tile_k_vec = 0u; tile_k_vec < TILE_K_VECS; tile_k_vec = tile_k_vec + 1u) {
            ...
            acc_3 = fma(vec4<f32>(input_0.x), weight_10, acc_3);
            acc_4 = fma(vec4<f32>(input_1.x), weight_10, acc_4);
            acc_5 = fma(vec4<f32>(input_2.x), weight_10, acc_5);
            ...
            acc_3 = fma(vec4<f32>(input_0.y), weight_11, acc_3);
            acc_4 = fma(vec4<f32>(input_1.y), weight_11, acc_4);
            acc_5 = fma(vec4<f32>(input_2.y), weight_11, acc_5);
            ...
            acc_3 = fma(vec4<f32>(input_0.z), weight_12, acc_3);
            acc_4 = fma(vec4<f32>(input_1.z), weight_12, acc_4);
            acc_5 = fma(vec4<f32>(input_2.z), weight_12, acc_5);
            ...
            acc_3 = fma(vec4<f32>(input_0.w), weight_13, acc_3);
            acc_4 = fma(vec4<f32>(input_1.w), weight_13, acc_4);
            acc_5 = fma(vec4<f32>(input_2.w), weight_13, acc_5);
        }
        workgroupBarrier();
    }
    ...
    store_result(row_0, column_1, acc_3);
    store_result(row_1, column_1, acc_4);
    store_result(row_2, column_1, acc_5);
}
