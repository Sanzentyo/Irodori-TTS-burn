...
// scores, softmax state, and accumulators remain f32.
...
@group(0) @binding(3) var<storage, read_write> mask_buf: array<f16>;
...
// Shared memory
var<workgroup> q_tile: array<f32, {{ q_tile_size }}>;
var<workgroup> kv_tile: array<f32, {{ kv_tile_size }}>;
...
fn main(
    ...
    for (var p = tid; p < Q_ELEMS; p = p + WG_SIZE) {
        ...
        let gq_r = q_base + r;
        if (gq_r < S_Q) {
            q_tile[r * D_PAD + d] = f32(q_buf[q_head_off + gq_r * D + d]);
        } else {
    ...
    for (var kb = 0u; kb < NUM_KV_BLOCKS; kb = kb + 1u) {
        ...
        for (var p = tid; p < KV_ELEMS; p = p + WG_SIZE) {
            ...
            let gkv = kv_base + r;
            if (gkv < S_KV) {
                kv_tile[r * D_PAD + d] = f32(k_buf[kv_head_off + gkv * D + d]);
            } else {
        ...
        if (valid_q && gkv < S_KV) {
            ...
            let mask_val = f32(mask_buf[mask_off + gkv]);
        ...
        for (var p = tid; p < KV_ELEMS; p = p + WG_SIZE) {
            ...
            let gkv = kv_base + r;
            if (gkv < S_KV) {
                kv_tile[r * D_PAD + d] = f32(v_buf[kv_head_off + gkv * D + d]);
            } else {
                kv_tile[r * D_PAD + d] = 0.0;
            }
        }
    ...
    if (valid_q) {
        ...
        if (sum > 0.0) {
            let inv_sum = 1.0 / sum;
            for (var d = 0u; d < DPT; d = d + 1u) {
                output_buf[out_base + sec + d * TILE_KV] = f16(out_accum[d] * inv_sum);
            }
        } else {
            for (var d = 0u; d < DPT; d = d + 1u) {
                output_buf[out_base + sec + d * TILE_KV] = f16(0.0);
            }
        }
    }
}
