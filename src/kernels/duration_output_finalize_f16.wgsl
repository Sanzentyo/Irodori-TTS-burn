...
@group(0) @binding(0) var<storage, read_write> hidden: array<f16>;
@group(0) @binding(1) var<storage, read_write> norm_weight: array<f16>;
@group(0) @binding(2) var<storage, read_write> output_weight: array<f16>;
@group(0) @binding(3) var<storage, read_write> output_bias: array<f16>;
...
fn main(@builtin(local_invocation_id) local_id: vec3<u32>) {
    ...
    if (token < SEQUENCE) {
        ...
        for (var feature = lane; feature < DIM; feature = feature + LANES) {
            let value = f32(hidden[row + feature]);
    ...
    if (tid == 0u) {
        ...
        output[0] = f16(log(1.0 + total_frames));
    }
}
