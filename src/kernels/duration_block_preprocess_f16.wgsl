...
@group(0) @binding(2) var<storage, read_write> scale: array<f16>;
@group(0) @binding(3) var<storage, read_write> shift: array<f16>;
...
fn main(
    ...
    for (var feature = tid; feature < DIM; feature = feature + 256u) {
        let value = f32(input[row + feature]);
    ...
    for (var feature = tid; feature < DIM; feature = feature + 256u) {
        ...
        output[index] = f16(f32(input[index]) * inv_rms * f32(norm_weight[feature]) * f32(scale[feature])
            + f32(shift[feature]));
    }
}
