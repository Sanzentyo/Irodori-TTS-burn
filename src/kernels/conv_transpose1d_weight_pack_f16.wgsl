...
@group(0) @binding(0) var<storage, read_write> source_weight: array<f16>;
@group(0) @binding(1) var<storage, read_write> packed_weight: array<f16>;
...
fn main(
    ...
    let packed_index =
        ...
        + tap;
    packed_weight[packed_index] = f16(f32(source_weight[source_index]));
}
