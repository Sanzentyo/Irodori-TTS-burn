...
@group(0) @binding(0) var<storage, read_write> input_nchw: array<f16>;
@group(0) @binding(1) var<storage, read_write> alpha: array<f16>;
@group(0) @binding(2) var<storage, read_write> output_nhwc: array<f16>;
...
fn main(
    ...
    for (var row = local_id.y; row < TILE; row += 8u) {
        ...
        if (channel < CHANNELS && time < TIME) {
            ...
            let x = f32(input_nchw[input_index]);
    ...
    for (var row = local_id.y; row < TILE; row += 8u) {
        ...
        if (output_channel < CHANNELS && output_time < TIME) {
            ...
            output_nhwc[output_index] =
                f16(activated_tile[local_id.x * TILE_STRIDE + row]);
        }
    }
}
