// Exact FP32 K16 MLP contract with alternating workgroup pages.
//
// The established route waits once after loading a tile and again before
// overwriting it. Separate even/odd pages make the overwrite independent of
// the tile still being consumed, leaving one uniform barrier per K slice.
...
const INPUT_PAGE_VECS: u32 = {{ input_tile_vecs }}u;
const WEIGHT_PAGE_VECS: u32 = {{ weight_tile_vecs }}u;

var<workgroup> input_tile: array<vec4<f32>, {{ double_input_tile_vecs }}>;
var<workgroup> weight_tile: array<vec4<f32>, {{ double_weight_tile_vecs }}>;
...
fn load_page(
    ...
    let input_page = page * INPUT_PAGE_VECS;
    let weight_page = page * WEIGHT_PAGE_VECS;
    for (var load = local_index; load < INPUT_PAGE_VECS; load = load + 256u) {
        ...
        input_tile[input_page + load] = value;
    }
    for (var load = local_index; load < WEIGHT_PAGE_VECS; load = load + 256u) {
        ...
        weight_tile[weight_page + load] = value;
    }
}
...
fn main(
    ...
    var page = 0u;
    load_page(page, 0u, row_base, column_vec_base, local_index);
    workgroupBarrier();
    ...
    for (var k_base = 0u; k_base < K; k_base = k_base + TILE_K) {
        ...
        for (var tile_k_vec = 0u; tile_k_vec < TILE_K_VECS; tile_k_vec = tile_k_vec + 1u) {
            let input_0 = input_tile[input_page + row_0 * TILE_K_VECS + tile_k_vec];
            let input_1 = input_tile[input_page + row_1 * TILE_K_VECS + tile_k_vec];
            let input_2 = input_tile[input_page + row_2 * TILE_K_VECS + tile_k_vec];
            let input_3 = input_tile[input_page + row_3 * TILE_K_VECS + tile_k_vec];
            let weight_base = weight_page + tile_k_vec * 4u * LOCAL_COLUMN_VECS + local_id.x;
        ...
        if (next_k < K) {
            page = 1u - page;
            load_page(page, next_k, row_base, column_vec_base, local_index);
            workgroupBarrier();
        }
    }
