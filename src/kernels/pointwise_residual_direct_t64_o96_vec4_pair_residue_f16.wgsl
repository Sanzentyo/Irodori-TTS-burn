...
// T64/O96 WG32x8 vector f32 pointwise + raw residual + next Snake in compact residue order. K templated.
...
@group(0) @binding(6) var<storage, read_write> activated_residue: array<f16>;
...
fn finish_raw(accumulator: f32, output_channel: u32, time: u32) -> f32 {
    ...
    return biased + f32(residual_ncl[output_channel * LENGTH + time]);
}

fn finish_snake(raw: f32, output_channel: u32) -> f32 {
    let a = f32(alpha[output_channel]);
    let sine = sin(a * raw);
    return raw + (sine * sine) / (a + 1e-9);
}
...
fn store_pair(accumulator: f32, output_channel: u32, time: u32) {
    ...
    let activated = finish_snake(raw, output_channel);
    ...
    let packed_index = residue_prefix_q * CHANNELS + output_channel * residue_length + q;
    activated_residue[packed_index] = f16(activated);
}

fn store_pair4(accumulator: vec4<f32>, output_channel: u32, time: u32) {
    if (time < LENGTH && output_channel < CHANNELS) {
        store_pair(accumulator.x, output_channel + 0u, time);
        store_pair(accumulator.y, output_channel + 1u, time);
        store_pair(accumulator.z, output_channel + 2u, time);
        store_pair(accumulator.w, output_channel + 3u, time);
    }
}
...
fn main(
    ...
    loop {
        ...
        loop {
            ...
            if (output_channel < CHANNELS) {
                ...
                weight_value =
                    vec4<f32>(packed_weight[input_channel * WEIGHT_VECTORS_PER_ROW + output_vector]);
            }
    ...
    store_pair4(accumulator_00, output_channel_base + 0u, time_0);
    store_pair4(accumulator_01, output_channel_base + 4u, time_0);
    store_pair4(accumulator_02, output_channel_base + 8u, time_0);
    store_pair4(accumulator_10, output_channel_base + 0u, time_1);
