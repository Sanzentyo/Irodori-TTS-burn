...
@group(0) @binding(2) var<storage, read_write> output_nlc: array<f16>;
...
fn main(
    ...
    loop {
        ...
        let x = f32(input_ncl[input_index]);
        let a = f32(alpha[input_channel]);
        let sine = sin(a * x);
    ...
    loop {
        ...
        output_nlc[output_index] =
            f16(activated_tile[local_id.x * TILE_STRIDE + time_lane]);
        repeat += 1u;
    }
}
