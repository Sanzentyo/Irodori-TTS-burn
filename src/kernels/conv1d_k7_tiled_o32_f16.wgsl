...
// Device-limit-gated T64/O32 kernel for the DACVAE residual k=7 Conv1d.
//
// Physical layouts:
...
//
// A 16x16 workgroup produces 64 time positions by 32 output channels. Each
// invocation owns four time positions and two output channels (eight f32
// accumulators). Reduction proceeds in input-channel-then-kernel order, which
// is identical to the portable T64/O16 production kernel.
//
...
fn main(
    ...
    if time_0 < LENGTH {
        output_buf[output_base_0 + time_0] = f16(accumulator_00);
        output_buf[output_base_1 + time_0] = f16(accumulator_01);
    }
    if time_1 < LENGTH {
        output_buf[output_base_0 + time_1] = f16(accumulator_10);
        output_buf[output_base_1 + time_1] = f16(accumulator_11);
    }
    if time_2 < LENGTH {
        output_buf[output_base_0 + time_2] = f16(accumulator_20);
        output_buf[output_base_1 + time_2] = f16(accumulator_21);
    }
    if time_3 < LENGTH {
        output_buf[output_base_0 + time_3] = f16(accumulator_30);
        output_buf[output_base_1 + time_3] = f16(accumulator_31);
    }
}
