...
// Long-sequence fused SwiGLU activation and w2 projection for the released duration predictor.

@group(0) @binding(0) var<storage, read_write> projected: array<f16>;
@group(0) @binding(1) var<storage, read_write> w2: array<vec4<f16>>;
...
fn main(
    ...
    for (var k_base = 0u; k_base < DIM; k_base = k_base + TILE_K) {
        for (var load = flat_tid; load < TILE_ROWS * TILE_K; load = load + 128u) {
            ...
            if (group_id.y * TILE_ROWS + activation_row < SEQUENCE) {
                ...
                let gate = f32(projected[source_row + k_base + activation_k]);
                let value = f32(projected[source_row + DIM + k_base + activation_k]);
        ...
        for (var load = flat_tid; load < TILE_K * TILE_OUTPUT_VECS; load = load + 128u) {
            ...
            weight_tile[load] = vec4<f32>(w2[(k_base + local_k) * DIM_VECS + global_output_vector]);
        }
        workgroupBarrier();
    ...
    for (var row_slot = 0u; row_slot < 2u; row_slot = row_slot + 1u) {
        let row = row_base + row_slot * 8u;
        if (row < SEQUENCE) {
            output[row * DIM_VECS + output_vector] = vec4<f16>(acc[row_slot]);
        }
    }
}
