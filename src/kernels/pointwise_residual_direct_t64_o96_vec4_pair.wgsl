// T64/O96 WG32x8 vector f32 pointwise + raw residual + next Snake. K templated.
...
@group(0) @binding(5) var<storage, read_write> raw_ncl: array<f32>;
@group(0) @binding(6) var<storage, read_write> activated_ncl: array<f32>;
...
fn store_pair(accumulator: f32, output_channel: u32, time: u32) {
    ...
    activated_ncl[output_index] = finish_snake(raw, output_channel);
}
