...
fn store_output_vec4(output_base: u32, time: u32, output_channel: u32, value: vec4<f32>) {
    if time < LENGTH {
        output_buf[output_base + time] = snake_epilogue(value.x, output_channel);
    }
    if time + 1u < LENGTH {
        output_buf[output_base + time + 1u] = snake_epilogue(value.y, output_channel);
    }
    if time + 2u < LENGTH {
        output_buf[output_base + time + 2u] = snake_epilogue(value.z, output_channel);
    }
    if time + 3u < LENGTH {
        output_buf[output_base + time + 3u] = snake_epilogue(value.w, output_channel);
    }
}
...
fn main(
    ...
    loop {
        ...
        // Flattening the shared tile as [O32, CinTile, K7] matches contiguous
        // checkpoint OIK and retains coalesced cooperative global reads.
