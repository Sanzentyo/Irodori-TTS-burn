...
// F16 released-WmHead fast path with an owned F32 consumer output.
// Arithmetic and reduction order match the F16-output kernel; only the final
// store skips the otherwise separate F16-to-F32 conversion dispatch.

@group(0) @binding(0) var<storage, read_write> input_ncl: array<f16>;
@group(0) @binding(1) var<storage, read_write> alpha: array<f16>;
@group(0) @binding(2) var<storage, read_write> weight_oik: array<f16>;
@group(0) @binding(3) var<storage, read_write> bias: array<f16>;
...
fn main(
    ...
    var accumulator = f32(bias[0u]);

    if (local_index < CHANNELS) {
        alpha_all[local_index] = f32(alpha[local_index]);
    }
    ...
    loop {
        ...
        weight_all[preload_weight_index] = f32(weight_oik[preload_weight_index]);
    ...
    loop {
        ...
        loop {
            ...
            if (source_time >= 0 && source_time < i32(TIME)) {
                ...
                let x = f32(input_ncl[input_channel * TIME + u32(source_time)]);
