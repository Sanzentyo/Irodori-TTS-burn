// Exact FP32 MLP contract with a 32-row occupancy tile, vectorized K staging,
// and a gated-residual epilogue. Compared with the 64-row route, each
// invocation retains four rather than eight vec4 accumulators and the launch
// exposes twice as many row workgroups. Ordered scalar FMAs are unchanged.
...
fn main(
    ...
    for (var k_base = 0u; k_base < K; k_base = k_base + TILE_K) {
        ...
        for (var tile_k_vec = 0u; tile_k_vec < TILE_K_VECS; tile_k_vec = tile_k_vec + 1u) {
            ...
            acc_2 = fma(vec4<f32>(input_0.x), weight_10, acc_2);
            acc_3 = fma(vec4<f32>(input_1.x), weight_10, acc_3);
            ...
            acc_2 = fma(vec4<f32>(input_0.y), weight_11, acc_2);
            acc_3 = fma(vec4<f32>(input_1.y), weight_11, acc_3);
            ...
            acc_2 = fma(vec4<f32>(input_0.z), weight_12, acc_2);
            acc_3 = fma(vec4<f32>(input_1.z), weight_12, acc_3);
            ...
            acc_2 = fma(vec4<f32>(input_0.w), weight_13, acc_2);
            acc_3 = fma(vec4<f32>(input_1.w), weight_13, acc_3);
        }
        workgroupBarrier();
    }
    ...
    store_result(row_0, column_1, acc_2);
    store_result(row_1, column_1, acc_3);
}
