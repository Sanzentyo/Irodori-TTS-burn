...
@group(0) @binding(3) var<storage, read_write> rope_cos: array<f16>;
@group(0) @binding(4) var<storage, read_write> rope_sin: array<f16>;
...
fn main(
    ...
    for (var dim = tid; dim < DH; dim = dim + BLOCK_SIZE) {
        let q_value = f32(fused_qkv[q_base + dim]);
        let k_value = f32(fused_qkv[k_base + dim]);
    ...
    for (var pair = tid; pair < HALF_DH; pair = pair + BLOCK_SIZE) {
        let even = 2u * pair;
        let odd = even + 1u;
        let q_re = f32(fused_qkv[q_base + even]) / q_rms * f32(q_weight[weight_base + even]);
        let q_im = f32(fused_qkv[q_base + odd]) / q_rms * f32(q_weight[weight_base + odd]);
        let k_re = f32(fused_qkv[k_base + even]) / k_rms * f32(k_weight[weight_base + even]);
        let k_im = f32(fused_qkv[k_base + odd]) / k_rms * f32(k_weight[weight_base + odd]);
        ...
        if (rotate) {
            ...
            q_out[out_base + even] = f16(q_re);
            q_out[out_base + odd] = f16(q_im);
            k_out[out_base + even] = f16(k_re);
            k_out[out_base + odd] = f16(k_im);
        }

        v_out[out_base + even] = f16(f32(fused_qkv[v_base + even]));
        v_out[out_base + odd] = f16(f32(fused_qkv[v_base + odd]));

        if (WRITES_GATE) {
            let gate_base = fused_token_base + 3u * KV_DIM + head_offset;
            let gate_even = f32(fused_qkv[gate_base + even]);
            let gate_odd = f32(fused_qkv[gate_base + odd]);
            // Match Burn's sigmoid fallback: exp(-log(exp(-x) + 1)).
            fused_qkv[gate_base + even] = f16(exp(-log(exp(-gate_even) + 1.0)));
            fused_qkv[gate_base + odd] = f16(exp(-log(exp(-gate_odd) + 1.0)));
        }
    }
}
