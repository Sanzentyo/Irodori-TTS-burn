...
fn main(
    ...
    if (output_row_0 < ROWS && output_column_vec < N_VECS) {
        output[output_row_0 * N_VECS + output_column_vec] = vec4<f16>(acc_0);
    }
    if (output_row_1 < ROWS && output_column_vec < N_VECS) {
        output[output_row_1 * N_VECS + output_column_vec] = vec4<f16>(acc_1);
    }
    if (output_row_2 < ROWS && output_column_vec < N_VECS) {
        output[output_row_2 * N_VECS + output_column_vec] = vec4<f16>(acc_2);
    }
    if (output_row_3 < ROWS && output_column_vec < N_VECS) {
        output[output_row_3 * N_VECS + output_column_vec] = vec4<f16>(acc_3);
    }
    if (output_row_4 < ROWS && output_column_vec < N_VECS) {
        output[output_row_4 * N_VECS + output_column_vec] = vec4<f16>(acc_4);
    }
    if (output_row_5 < ROWS && output_column_vec < N_VECS) {
        output[output_row_5 * N_VECS + output_column_vec] = vec4<f16>(acc_5);
    }
    if (output_row_6 < ROWS && output_column_vec < N_VECS) {
        output[output_row_6 * N_VECS + output_column_vec] = vec4<f16>(acc_6);
    }
    if (output_row_7 < ROWS && output_column_vec < N_VECS) {
        output[output_row_7 * N_VECS + output_column_vec] = vec4<f16>(acc_7);
    }
}
