...
@group(0) @binding(3) var<storage, read_write> norm_weight: array<f32>;
@group(0) @binding(4) var<storage, read_write> output_weight: array<f32>;
@group(0) @binding(5) var<storage, read_write> output_bias: array<f32>;
...
fn main(@builtin(local_invocation_id) local_id: vec3<u32>) {
    ...
    if (token < SEQUENCE) {
        ...
        for (var feature = lane; feature < DIM; feature = feature + LANES) {
            ...
            let value = fma(gate[feature], branch[index], residual[index]);
    ...
    if (tid == 0u) {
        output[0] = log(1.0 + max(square_parts[0], 0.0));
    }
}
