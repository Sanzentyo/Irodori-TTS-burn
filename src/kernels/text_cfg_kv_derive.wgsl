...
@group(0) @binding(0) var<storage, read_write> conditional: array<f32>;
@group(0) @binding(1) var<storage, read_write> batched_cfg: array<f32>;
...
fn main(@builtin(global_invocation_id) global_id: vec3<u32>) {
    ...
    if (batch == 0u) {
        batched_cfg[output_index] =
            conditional[plane * CONTEXT_ELEMENTS + context_index];
    } else {
        batched_cfg[output_index] = 0.0;
    }
}
