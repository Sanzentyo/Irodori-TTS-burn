...
// Production T256/O32 DACVAE residual k=7 Conv1d + act1 Snake epilogue.
//
// The accepted T256 convolution reduction is unchanged; the fifth alpha
// binding and scalar Snake operation are applied only in the guarded store.
//
// Physical layouts:
...
// Scalar guarded stores retain the complete measured time-tail behavior.
fn store_output_vec4(output_base: u32, time: u32, output_channel: u32, value: vec4<f32>) {
    if time < LENGTH {
        output_buf[output_base + time] = f16(snake_epilogue(value.x, output_channel));
    }
    if time + 1u < LENGTH {
        output_buf[output_base + time + 1u] = f16(snake_epilogue(value.y, output_channel));
    }
    if time + 2u < LENGTH {
        output_buf[output_base + time + 2u] = f16(snake_epilogue(value.z, output_channel));
    }
    if time + 3u < LENGTH {
        output_buf[output_base + time + 3u] = f16(snake_epilogue(value.w, output_channel));
    }
}
