...
@group(0) @binding(1) var<storage, read_write> packed_weight_buf: array<f16>;
...
fn main(
        ...
        loop {
            ...
            let packed_index =
                (((phase * OUTPUT_CHANNELS + output_channel_base + tile_output_channel)
                    ...
                    * 2u)
                + tap;
            weight_tile[tile_index] = f32(packed_weight_buf[packed_index]);
            tile_index += WORKGROUP_SIZE;
        }
    ...
    let bias = f32(bias_buf[output_channel]);
    ...
    if (time_0 < INPUT_LENGTH) {
        ...
        output_buf[output_index] = f16(accumulator_main_0 + accumulator_previous_0 + bias);
    }
    if (time_1 < INPUT_LENGTH) {
        let output_index = output_channel * OUTPUT_LENGTH + time_1 * STRIDE + output_phase;
        output_buf[output_index] = f16(accumulator_main_1 + accumulator_previous_1 + bias);
    }
    if (time_2 < INPUT_LENGTH) {
        let output_index = output_channel * OUTPUT_LENGTH + time_2 * STRIDE + output_phase;
        output_buf[output_index] = f16(accumulator_main_2 + accumulator_previous_2 + bias);
    }
    if (time_3 < INPUT_LENGTH) {
        let output_index = output_channel * OUTPUT_LENGTH + time_3 * STRIDE + output_phase;
        output_buf[output_index] = f16(accumulator_main_3 + accumulator_previous_3 + bias);
    }
}
