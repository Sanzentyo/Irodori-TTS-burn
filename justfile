...
# Inference with LibTorch MPS f16 backend
infer-real-tch-mps-f16 *args:
    LIBTORCH_USE_PYTORCH=1 \
    LIBTORCH_BYPASS_VERSION_CHECK=1 \
    VIRTUAL_ENV={{ PYTHON_VENV }} \
    PATH={{ PYTHON_VENV_BIN }}:{{ TORCH_LIB_DIR }}:{{ SYSTEM_PATH }} \
...
# Inference with WgpuRaw f16 backend
infer-real-raw-f16 *args:
    cargo run --release --features cli --bin infer -- \
        --backend wgpu-raw-f16 \
        --checkpoint target/model_converted.safetensors {{ args }}

# ── Quality Comparison ────────────────────────────────────────────────────────
# Generate latent outputs for sampler quality comparison (uses fixed noise for reproducibility).
# Runs Euler-40/25, Heun-20, PLMS4-20/25 all with --noise-file, then compares latents.

# Usage: just quality-compare-latents [--text "テキスト"] [--noise-file /tmp/initial_noise.safetensors]
quality-compare-latents text="テスト用音声を合成します" noise="/tmp/initial_noise.safetensors" outdir="/tmp/quality_cmp":
    #!/usr/bin/env bash
    set -euo pipefail
    mkdir -p "{{ outdir }}"
    echo "=== Generating latents for quality comparison ==="
    echo "Text: {{ text }}"
    echo "Noise: {{ noise }}"
    echo ""
    ...
    VIRTUAL_ENV="{{ PYTHON_VENV }}" \
    PATH="{{ PYTHON_VENV_BIN }}:{{ TORCH_LIB_DIR }}:{{ SYSTEM_PATH }}" \
    DYLD_LIBRARY_PATH="{{ TORCH_LIB_DIR }}:${DYLD_LIBRARY_PATH:-}" \
    cargo run --release --features cli --bin infer -- \
        --backend libtorch-mps-f16 --checkpoint target/model_converted.safetensors \
        --sampler euler --num-steps 40 --noise-file "{{ noise }}" \
        --text "{{ text }}" --output "{{ outdir }}/euler40.safetensors"
    echo "✓ euler-40 done"
    LIBTORCH_USE_PYTORCH=1 LIBTORCH_BYPASS_VERSION_CHECK=1 \
    VIRTUAL_ENV="{{ PYTHON_VENV }}" \
    PATH="{{ PYTHON_VENV_BIN }}:{{ TORCH_LIB_DIR }}:{{ SYSTEM_PATH }}" \
    ...
    cargo run --release --features cli --bin infer -- \
        --backend libtorch-mps-f16 --checkpoint target/model_converted.safetensors \
        --sampler euler --num-steps 25 --noise-file "{{ noise }}" \
        --text "{{ text }}" --output "{{ outdir }}/euler25.safetensors"
    echo "✓ euler-25 done"
    LIBTORCH_USE_PYTORCH=1 LIBTORCH_BYPASS_VERSION_CHECK=1 \
    VIRTUAL_ENV="{{ PYTHON_VENV }}" \
    PATH="{{ PYTHON_VENV_BIN }}:{{ TORCH_LIB_DIR }}:{{ SYSTEM_PATH }}" \
    ...
    cargo run --release --features cli --bin infer -- \
        --backend libtorch-mps-f16 --checkpoint target/model_converted.safetensors \
        --sampler heun --num-steps 20 --noise-file "{{ noise }}" \
        --text "{{ text }}" --output "{{ outdir }}/heun20.safetensors"
    echo "✓ heun-20 done"
    LIBTORCH_USE_PYTORCH=1 LIBTORCH_BYPASS_VERSION_CHECK=1 \
    VIRTUAL_ENV="{{ PYTHON_VENV }}" \
    PATH="{{ PYTHON_VENV_BIN }}:{{ TORCH_LIB_DIR }}:{{ SYSTEM_PATH }}" \
    ...
    cargo run --release --features cli --bin infer -- \
        --backend libtorch-mps-f16 --checkpoint target/model_converted.safetensors \
        --sampler plms4 --num-steps 20 --noise-file "{{ noise }}" \
        --text "{{ text }}" --output "{{ outdir }}/plms4_20.safetensors"
    echo "✓ plms4-20 done"
    LIBTORCH_USE_PYTORCH=1 LIBTORCH_BYPASS_VERSION_CHECK=1 \
    VIRTUAL_ENV="{{ PYTHON_VENV }}" \
    PATH="{{ PYTHON_VENV_BIN }}:{{ TORCH_LIB_DIR }}:{{ SYSTEM_PATH }}" \
    ...
    cargo run --release --features cli --bin infer -- \
        --backend libtorch-mps-f16 --checkpoint target/model_converted.safetensors \
        --sampler plms4 --num-steps 25 --noise-file "{{ noise }}" \
        --text "{{ text }}" --output "{{ outdir }}/plms4_25.safetensors"
    echo "✓ plms4-25 done"
    echo ""
    echo "=== Comparing latents ==="
    cd ../Irodori-TTS && \
    .venv/bin/python ../Irodori-TTS-burn/scripts/compare_latents.py \
        --ref "{{ outdir }}/euler40.safetensors" \
        "{{ outdir }}/euler25.safetensors" \
        "{{ outdir }}/heun20.safetensors" \
        "{{ outdir }}/plms4_20.safetensors" \
        "{{ outdir }}/plms4_25.safetensors"
