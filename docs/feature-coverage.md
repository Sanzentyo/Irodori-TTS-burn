...
| `src/weights/` | 26 tests | TensorEntry validation, f32/bf16/f16 decode, roundtrip encode/decode, TensorStore load, linear transpose, linear with/without bias, embedding, rms_norm, missing weight errors; **integration**: build_model_record speaker/caption mode, wrong-config error, sentinel round-trip; **LoRA**: merge math round-trip, PEFT prefix strip, end-to-end load, BF16 round-trip, missing-key silence, wrong-rank error |
...
| All library features (`inference,codec,text-normalization,lora,train`) | **238** |
