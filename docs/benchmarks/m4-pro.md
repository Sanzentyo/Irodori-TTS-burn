...
10. **Algorithmic optimizations (LibTorch MPS f16)** — all vs Euler-100 ground truth:
    - Euler-40: RTF=0.379, PSNR=43.25 dB (reference quality)
    - Heun-20 (NFE=40): RTF=0.369, PSNR=41.58 dB — same NFE as Euler-40
    - **Euler-25**: RTF=0.241, PSNR=40.44 dB — **best speed-quality tradeoff** (−37% vs Euler-40)
    - Heun-10 (NFE=20): RTF=0.181, PSNR=38.77 dB — marginally better than Euler-20 at same NFE
    - Euler-20: RTF=0.190, PSNR=38.52 dB
    - **PLMS4 not recommended**: Euler wins at every tested step count (matched NFE comparison)
...
- Quality: see "Sampler Quality Comparison (Latent-Space)" section below
...
### Euler-25 Benchmark (LibTorch MPS f16)

| Configuration | Steps | NFE | Mean (ms) | RTF | vs Euler-40 |
|---|---|---|---|---|---|
| Euler (baseline) | 40 | 40 | 11,371 | 0.379 | 1.0× |
| Euler | 25 | 25 | 7,228 | **0.241** | **−36%** |

Note: Euler-25 and PLMS4-25 have identical RTF (0.241) but Euler-25 has significantly higher quality
vs Euler-100 ground truth (PSNR=40.44 dB vs 37.73 dB). Euler-25 is strongly recommended over PLMS4-25.

### Sampler Quality Comparison (Latent-Space, LibTorch MPS f16)

**Method**: All samplers used identical initial noise from `scripts/export_initial_noise.py`
(seed=42, [1,750,32] f32 safetensors). Reference is **Euler-100** (100-step high-accuracy
approximation used as ground truth). Latents generated by
`just infer-real-tch-mps-f16 --noise-file /tmp/initial_noise.safetensors`. No DACVAE
decoding — latent-space comparison avoids DACVAE reconstruction and tail-trimming noise.

**Note**: PSNR/CosSim values measure *agreement with Euler-100*, not absolute audio quality.
CosSim > 0.999 indicates strong latent similarity, but perceptual equivalence requires decode + listening.
Single prompt, single seed — multi-prompt/seed matrix not yet run.

| Sampler | Steps | NFE | RTF | MAE | CosSim | PSNR vs E-100 |
|---|---|---|---|---|---|---|
| Euler-100 (ref) | 100 | 100 | 0.947 | — | 1.0 | ∞ |
| Euler-40 | 40 | 40 | 0.379 | 0.01830 | 0.999719 | **43.25 dB** |
| Heun-20 | 20 | 40 | 0.369 | 0.02152 | 0.999618 | **41.58 dB** |
| Euler-25 | 25 | 25 | 0.241 | 0.02406 | 0.999545 | **40.44 dB** |
| Heun-10 | 10 | 20 | 0.181 | 0.02895 | 0.999347 | 38.77 dB |
| Euler-20 | 20 | 20 | 0.190 | 0.02940 | 0.999399 | 38.52 dB |
| PLMS4-25 | 25 | 25 | 0.241 | 0.02882 | 0.999241 | 37.73 dB |
| PLMS4-20 | 20 | 20 | ~0.193 | 0.03341 | 0.998856 | 36.44 dB |
| Euler-15 | 15 | 15 | ~0.142 | 0.03645 | 0.999105 | 36.20 dB |
| PLMS4-15 | 15 | 15 | ~0.142 | 0.03705 | 0.998208 | 35.02 dB |

**Key findings**:
1. **Euler consistently outperforms PLMS4 at every tested step count** (matched NFE comparison):
   - Euler-25 vs PLMS4-25: +2.71 dB (40.44 vs 37.73 dB)
   - Euler-20 vs PLMS4-20: +2.08 dB (38.52 vs 36.44 dB)
   - Euler-15 vs PLMS4-15: +1.18 dB (36.20 vs 35.02 dB)
2. **PLMS4-20 ≈ Euler-15 in quality** (36.44 vs 36.20 dB) — PLMS4 costs ~5 extra steps for no gain.
3. **Heun-10 (20 NFE) is marginally better than Euler-20 (20 NFE)** by 0.25 dB — effectively equivalent.
4. **Heun-20 (40 NFE) is 1.67 dB below Euler-40 (40 NFE)** — half iterations, slightly lower quality.
5. **Euler-25 is the best speed-quality tradeoff** at RTF=0.241 (−37% vs Euler-40, PSNR=40.44 dB).

**Why PLMS4 underperforms Euler here**:
- AB startup cost: first 3 steps use AB1→AB3 (lower order) — accounts for 12-20% of short runs
- Piecewise-smooth RHS: `cfg_min_t` boundary creates a non-smooth forcing term that breaks
  the Adams-Bashforth order guarantee (AB4 is 4th-order on smooth ODEs only)
- Speaker-KV cache transition adds another piecewise discontinuity

**Production recommendations**:
- **Best quality (standard)**: Euler-40 (RTF=0.379, PSNR=43.25 dB vs E-100)
- **Best speed+quality**: Euler-25 (RTF=0.241, PSNR=40.44 dB) — **recommended default**
- **Faster, near-equivalent to Euler-20**: Heun-10 (RTF=0.181, PSNR=38.77 dB)
- **Not recommended**: PLMS4 at any step count — Euler at same NFE always wins
- Script: `just quality-compare-latents` for reproducible latent-space comparison
