...
| Rust/burn Wgpu (f32, fusion) | 7,315 | — | 7,298 | 7,318 | 7,331 | 2.72× |
| Rust/burn Wgpu-f16 | ⚠️ NOT YET MEASURED | — | — | — | — | — |
| Rust/burn WgpuRaw (no fusion, f32) | ❌ INVALID (GPU occupied) | — | — | — | — | — |
...
> ⚠️ **WgpuRaw benchmark DISCARDED**: Run attempted 2026-07-21 while GPU was occupied by
> another process. Result (10,422ms) is invalid. Must re-run on idle GPU.
> Rubber duck review also suggests WgpuRaw strategy may not be viable without compensating
> for fusion-layer loss (~30%). See `docs/planning/wgpu-optimization.md` for analysis.

### Hardware No Longer Available (2026-07-22)

The A6000 machine has been decommissioned. The questions below were answered by later
measurements on the RTX 5070 Ti (laptop) and M4 Pro Mac Mini — see `docs/benchmarks/`:

- **Wgpu-f16 on NVIDIA**: RTX 5070 Ti measured 4,538ms (f16) vs 6,720ms (f32) — 32% faster.
  Likely similar or better on A6000, but still 1.6–2.3× Python f32.
- **WgpuRaw vs Wgpu fusion overhead**: RTX 5070 Ti WgpuRaw = 7,049ms vs Wgpu = 6,720ms.
  Fusion saves only ~5%, not the estimated 30%. Custom norm kernels address <6% of WGPU time
  and are not worth integrating. See `docs/benchmarks/rtx-5070ti-laptop.md`.
- **WgpuRaw custom RMSNorm**: WGPU operator profile (DX12) — matmul=71.7%, SDPA=22.6%.
  Norm kernels address <6%. No integration benefit.
- **Realistic text_len**: Covered by `docs/benchmarks/m4-pro.md` where all E2E measurements
  use realistic inputs (text_len via `ModelConfig::default()`).
