...
### TODO: Measurements Needed (next session)

- [ ] Wgpu-f16 benchmark (may be significantly faster on GPU with shader-f16 support)
- [ ] WgpuRaw f32 on idle GPU (needed to assess fusion vs no-fusion overhead)
- [ ] WgpuRaw with custom RMSNorm kernel vs default RMSNorm (isolated kernel benchmark)
